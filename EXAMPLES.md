...
## 1.5 Notification si des zones sont contournées à l'armement

```yaml
automation:
  - id: notification_zones_contournees
    alias: "Notification - Zones contournées"
    ...
    action:
      - service: notify.mobile_app_votre_telephone
        data:
          title: "⚠️ Alarme armée avec protection réduite"
          message: >-
            Zones temporairement contournées :
            {{ trigger.event.data.bypassed_sensors | join(', ') }}.
            Elles seront réintégrées automatiquement après leur fermeture.
          data:
            priority: high
            channel: alarm
```
