...
- Nom convivial du capteur responsable dans l'attribut `triggered_by_name` et l'événement `alarme_personnalisee.triggered`.
- Notification système Home Assistant indiquant le nom et l'identifiant du capteur responsable au déclenchement.
- Bandeau visible dans la carte Lovelace pendant le délai d'entrée et le déclenchement.
- Exemples de notifications indiquant le nom et l'identifiant du capteur responsable.
- Couverture de tests supérieure à 95 %, avec le flux de configuration entièrement couvert et un seuil CI fixé à 95 %.

## [1.6.1] - 2026-07-29

### Modifié

- L'armement continue lorsqu'une zone du mode demandé est ouverte, inconnue, indisponible ou absente.
- Ces zones sont temporairement contournées, puis réintégrées automatiquement dès leur retour à `off`.
- Une ouverture ultérieure d'une zone réintégrée déclenche normalement le délai d'entrée.

### Ajouté

- Attribut `bypassed_sensors`, alerte Repairs et événement `alarme_personnalisee.bypassed_sensors_changed`.
- Affichage des zones contournées dans la carte Lovelace.
- Tests du cycle complet ouverture, contournement, fermeture, réintégration et nouvelle ouverture.

## [1.6.0] - 2026-07-29

### Ajouté

- Restauration du mode armé, du compteur et des dernières informations après un redémarrage.
- Alerte Repairs lorsque des zones ou lecteurs de badge sont absents, inconnus ou indisponibles.
- Événement `alarme_personnalisee.sensor_availability_changed` utilisable pour les notifications personnelles.
- Carte Lovelace `custom:alarme-personnalisee-card` responsive, avec commandes, PIN temporaire, statistiques et état des zones.
- Tests de restauration et de cycle de vie des alertes Repairs.

### Sécurité

- Les états transitoires (`arming`, `pending`, `triggered`) ne reprennent pas un ancien minuteur après redémarrage : l'intégration restaure directement le dernier mode armé connu.
- Une alerte de niveau erreur signale immédiatement une protection réduite due à un capteur indisponible.

## [1.5.0] - 2026-07-29

### Corrigé

- Compatibilité avec la propriété `alarm_state` de Home Assistant 2025.11+.
- Valeur `datetime` valide pour le capteur d'horodatage.
- Refus d'armer lorsqu'un capteur surveillé est déjà actif.
- Vérification exacte de la valeur des badges provenant d'un lecteur `sensor`.
...
- Valeur par défaut correcte de 180 secondes pour la durée de déclenchement.
- Cycle de vie du service de réinitialisation et des abonnements.
- Encodage UTF-8 des fichiers du dépôt.

### Ajouté

- Tests fonctionnels automatisés et contrôle Ruff.
- Noms traduits et catégories pour les entités.
- Événements `alarme_personnalisee.armed` et `alarme_personnalisee.disarmed`.

### Supprimé

- Ancien panneau HTML non enregistré et non fonctionnel.
- Module de services dupliqué et invalide.
